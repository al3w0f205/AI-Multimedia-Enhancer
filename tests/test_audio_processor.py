...
class TestAudioProcessor:
    ...
    def test_process_audio_only(self, mock_apply_dfn):
        ...
        mock_apply_dfn.assert_called_once_with("input.mp3", "output.mp3", 50.0, True, False, 0.0, "mp3", "192k")

    @patch("src.audio_processor.subprocess.run")
    @patch("src.audio_processor.VideoFileClip")
    @patch("src.audio_processor.AudioFileClip")
    @patch("src.audio_processor.AudioProcessor._apply_deepfilternet")
    def test_process_video(self, mock_apply_dfn, mock_audio_clip, mock_video_clip, mock_subprocess_run):
        processor = AudioProcessor()
        ...
        # Simular que subprocess.run funciona correctamente
